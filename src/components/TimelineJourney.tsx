...
export default function TimelineJourney({ experiences, portfolioData }: TimelineJourneyProps) {
  ...
  return (
    <section id="journey" className="w-full py-24 border-b border-slate-200 dark:border-white/5">
          ...
          {filters.map((f) => (
            <button
              ...
              className={`px-4.5 py-2.5 rounded-full text-xs font-sans tracking-wide font-medium transition-all cursor-pointer ${
                activeFilter === f
                  ? `${colors.bg} text-white dark:text-black font-bold shadow-lg shadow-orange-500/20`
      ...
      <div className="relative pl-6 sm:pl-10 border-l border-slate-200 dark:border-white/10 space-y-10 w-full py-2">
        {filteredExperiences.map((exp) => (
          ...
          >
            {/* Timeline Ring indicator centered exactly on the vertical line */}
            <span className={`absolute left-[-24px] sm:left-[-40px] -translate-x-1/2 top-4 w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center border-2 ${colors.indicator} shadow-lg group-hover:scale-105 transition-transform duration-300 z-10`}>
              {getExperienceIcon(exp.type)}
            </span>

            {/* Premium Glass Card envelope */}
            <div className="premium-card p-6 sm:p-8 rounded-2xl w-full">
